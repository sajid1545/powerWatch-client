...
const beginButtonLoading=button=>{if(!button||button.dataset.apiPending)return;button.dataset.apiPending='true';button.classList.add('api-pending');button.disabled=true};
const endButtonLoading=button=>{if(!button?.dataset.apiPending)return;button.classList.remove('api-pending');delete button.dataset.apiPending;button.disabled=false};
